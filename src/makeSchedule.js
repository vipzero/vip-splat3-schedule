...
export async function makeScheduleImgBuf(req, res) {
  const scheduleData = await getSchedule()
  return await makeImage(scheduleData)
}
...
async function makeImage(scheduleData) {
  const canvas = await makeAllImg(scheduleData)
...
const OpenStageImg = async (schedule_json) => {
  registerFont("./public/Koruri-20210720/Koruri-Regular.ttf", {
    family: "kokuri_r",
  })
  registerFont("./public/Koruri-20210720/Koruri-Extrabold.ttf", {
    family: "kokuri_exb",
  })

  const one_schedule_h = 160
  const all_schedule_h =
    one_schedule_h * schedule_json.result.bankara_open.length
  ...
  for (const match of schedule_json.result.bankara_open) {
    ...
    ctx_om.fillStyle =
      schedule_index % 2 === 0 ? "rgb(169,169,169)" : "rgb(220,220,220)"
    ...
    for (const stage of match.stages) {
      ...
      imgbuff = await loadImage(
        "./public/asset/stage_vss/stage_" + ("0" + stage.id).slice(-2) + ".png"
      )
      ...
      stg_ctx.font = '40px "kokuri_exb'

      imgbuff = await loadImage(
        `./public/asset/rule/${getRuleAsset(match.rule.key)}`
      )
...
const challengeStageImg = async (schedule_json) => {
  ...
  const one_schedule_h = 160
  const all_schedule_h =
    one_schedule_h * schedule_json.result.bankara_challenge.length
  const rule_size = 120
  const canvas = createCanvas(640, all_schedule_h)
  const canvas_om = createCanvas(640, 160)
  const ctx = canvas.getContext("2d")
...
const scheduleTimeImg = async (schedule_json) => {
  ...
  for (const match of schedule_json.result.bankara_open) {
    ...
    const makedate =
      year + "/" + month + "/" + day + " " + dayOfWeekStr[date.getDay()]
...
const makeAllImg = async (schedule_json) => {
  ...
  ctx.font = '25px "kokuri_exb'
  ctx.fillStyle = "rgb(0, 0, 0)"
  ctx.fillText("gen: " + new Date().toLocaleString(), 10, 1905)
  return canvas
}
